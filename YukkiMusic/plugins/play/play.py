    ...
    [
        [
            InlineKeyboardButton(
                text="اشترك هنا", url="https://t.me/ASMAA_NWESAR_LECTURES"
            ),                        
        ],        
    ]
)
...
async def check_is_joined(message):    
    try:
        ...
        status = await app.get_chat_member("ASMAA_NWESAR_LECTURES", userid)
        return True
    except Exception:
        await message.reply_text("*انت لست مشترك في قناة البوت @ASMAA_NWESAR_LECTURES ** \n**انضم لتستطيع تشغيل الاغاني**",reply_markup=force_btn,parse_mode="markdown",disable_web_page_preview=False)
        return False
